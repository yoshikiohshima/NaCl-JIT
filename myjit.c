...
/** @file myjit.c
 * This example is based on hello_world of Native Client, but 
 * demonstrates to call a function in the dynamic code region.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static const char* const kCallFooMethodId = "callFoo";

static char msg[200];
struct PP_Var CallFoo();
...
static bool HelloWorld_HasMethod(void* object,
  ...
  if (NULL != method_name) {
    if (strcmp(method_name, kCallFooMethodId) == 0) {
      return true;
    }
  } else {
...
void copy_and_pad_fragment(void *dest,
  ...
  fill_nops((uint8_t*)dest, dest_size);
...
CallFoo()
{
  ...
  copy_and_pad_fragment(buf, sizeof(buf), (const char*)foo, (const char *)(((char*)foo)+15));
